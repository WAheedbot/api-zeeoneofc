...
var creatorList = ['@johannes','@alpha'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/12086753126 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'JOHANNES') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'JOHANNES') return res.json(loghandler.invalidKey)
